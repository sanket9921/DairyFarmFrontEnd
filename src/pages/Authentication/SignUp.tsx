...
const SignUp: React.FC = () => {
  ...
  const checkUserExists = async (): Promise<boolean> => {
    try {
      const response = await UserService.checkUsernameOrEmailExists(username, email);
      return response.data; // true if exists, false otherwise
    } catch (error) {
      console.error(error);
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userExists) {
      setExistsError('Username or Email already exists');
      return;
    } else {
      setExistsError(null); // Clear the error if it doesn't exist
    }
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        {errors.username && <p style={{ color: 'red' }}>{errors.username}</p>}
          ...
          {errors.password && <p style={{ color: 'red' }}>{errors.password}</p>}
                ...
                {existsError && <p style={{ color: 'red' }}>{existsError}</p>}
